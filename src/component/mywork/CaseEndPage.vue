<template>
    <div class="home-container">
        <header id="header" class="mui-bar mui-bar-nav">
            ...
            <a class="mui-title">结案审批表</a>
            <!-- <mt-button type="primary" class="mui-pull-right" @click="addCase">添加</mt-button> -->
            <a class="mui-pull-right mui-icon mui-icon-right-nav" @click="handle('123')" v-show="(formKey0 == formname)">添加</a>
        </header>
        <div class="card-container">
            <form class="mui-input-group">
                <div class="mui-input-row">
                    <label>被监察单位：</label>
                    <input type="text" placeholder="输入被监察单位" v-model="docVarList.ldbzjcjaspb_beijianchadanwei">
                </div>
            </form>
            <form class="mui-input-group">
                <div class="mui-input-row">
                    <label>案由：</label>
                    <input type="text" placeholder="输入案由" v-model="docVarList.ldbzjcjaspb_anyou">
                </div>
            </form>
            <form class="mui-input-group">
                <div class="mui-input-row" style="margin: 2px 0">
                    <textarea id="textarea" rows="5" placeholder="输入调查处理过程" v-model="docVarList.ldbzjcjaspb_diaochachuliguocheng"></textarea>
                </div>
                <div class="mui-input-row" style="margin: 2px 0">
                    <textarea id="textarea" rows="5" placeholder="输入处理处罚及执行情况" v-model="docVarList.ldbzjcjaspb_chulichufajizhixingqingkuang"></textarea>
                </div>
                <div class="mui-input-row" style="margin: 2px 0">
                    <textarea id="textarea" rows="5" placeholder="输入承办监察员意见" v-model="docVarList.ldbzjcjaspb_chengbanjianchuayuanyijian"></textarea>
                </div>
            </form>
            <form class="mui-input-group">
                <div class="mui-input-row">
                    <label>主办监察员：</label>
                    <input type="text" placeholder="输入主办监察员" v-model="docVarList.ldbzjcjaspb_zhubanjianchayuan">
                </div>
                <div class="mui-input-row" data-options='{"type":"date"}' @click="openPicker('Date1',$event)">
                    <label data-options='{"type":"date"}'>日期：</label>
                    <input type="text" id="Date1" data-options='{"type":"date"}' readonly="readonly" v-model="docVarList.ldbzjcjaspb_zhubanjianchayuanriqi ">
                </div>
            </form>
            <form class="mui-input-group">
                <div class="mui-input-row">
                    <label>协办监察员：</label>
                    <input type="text" placeholder="输入协办监察员" v-model="docVarList.ldbzjcjaspb_xiebanjianchayuan">
                </div>
                <div class="mui-input-row" data-options='{"type":"date"}' @click="openPicker('Date2',$event)">
                    <label data-options='{"type":"date"}'>日期：</label>
                    <input type="text" id="Date2" data-options='{"type":"date"}' readonly="readonly" v-model="docVarList.ldbzjcjaspb_xiebanjianchayuanriqi">
                </div>
            </form>
            <form class="mui-input-group">
                <div class="mui-input-row" style="margin: 2px 0">
                    <textarea id="textarea" rows="5" placeholder="输入审核人意见" v-model="docVarList.ldbzjcjaspb_shenheyijian"></textarea>
                </div>
                <div class="mui-input-row">
                    <label>审核人：</label>
                    <input type="text" placeholder="输入审核人" v-model="docVarList.ldbzjcjaspb_shenheren ">
                </div>
                <div class="mui-input-row" data-options='{"type":"date"}' @click="openPicker('Date3',$event)">
                    <label data-options='{"type":"date"}'>审核日期：</label>
                    <input type="text" id="Date3" data-options='{"type":"date"}' readonly="readonly" v-model="docVarList.ldbzjcjaspb_shenheriqi">
                </div>
            </form>
            <form class="mui-input-group">
                <div class="mui-input-row" style="margin: 2px 0">
                    <textarea id="textarea" rows="5" placeholder="输入审批人意见" v-model="docVarList.ldbzjcjaspb_shenpiyijian"></textarea>
                </div>
                <div class="mui-input-row">
                    <label>审批人：</label>
                    <input type="text" placeholder="输入审批人" v-model="docVarList.ldbzjcjaspb_shenpiren">
                </div>
                <div class="mui-input-row" data-options='{"type":"date"}' @click="openPicker('Date4',$event)">
                    <label data-options='{"type":"date"}'>审批日期：</label>
                    <input type="text" id="Date4" data-options='{"type":"date"}' readonly="readonly" v-model="docVarList.ldbzjcjaspb_shenpiriqi ">
                </div>
            </form>
            <form class="mui-input-group">
                <div class="mui-input-row" style="margin: 2px 0">
                    <textarea id="textarea" rows="5" placeholder="输入备注" v-model="docVarList.ldbzjcjaspb_beizhu"></textarea>
                </div>
            </form>
...
export default {
    data() {
        return {
            ...
            vars: [
                "ldbzjcjaspb_beijianchadanwei",
                "ldbzjcjaspb_anyou",
                "ldbzjcjaspb_diaochachuliguocheng",
                "ldbzjcjaspb_chulichufajizhixingqingkuang",
                "ldbzjcjaspb_chengbanjianchuayuanyijian",
                "ldbzjcjaspb_zhubanjianchayuan",
                "ldbzjcjaspb_zhubanjianchayuanriqi",
                "ldbzjcjaspb_xiebanjianchayuan",
                "ldbzjcjaspb_xiebanjianchayuanriqi",
                "ldbzjcjaspb_shenheyijian",
                "ldbzjcjaspb_shenheren",
                "ldbzjcjaspb_shenheriqi",
                "ldbzjcjaspb_shenpiyijian",
                "ldbzjcjaspb_shenpiren",
                "ldbzjcjaspb_shenpiriqi",
                "ldbzjcjaspb_beizhu" 
            ],
            CASE_ID_: "",
            TASK_ID_: "案件结案",
