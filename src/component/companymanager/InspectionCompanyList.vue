<template>
    <div class="home-container">
        <header id="header" class="mui-bar mui-bar-nav">
            ...
            <a class="mui-title">企业年检</a>
            <a class="mui-icon mui-icon-plusempty mui-pull-right" @click="goAddInspectionCompany"></a>
        </header>
        <div class="card-container">
            <router-link class="mui-card" tag="div" v-for="(item,index) in inspectionList" :key="item.enterpriseID" :to="'/companymanager/inspectioncompanyinfo/'+index">
                <div class="mui-card-header">{{item.enterpriseName}}</div>
                <div class="mui-card-content">
                    <div class="mui-card-content-inner">
                        <p>公司名称：{{item.enterpriseName}}</p>
                        <p>单位法人：{{item.legalRepresentative}}</p>
                        <p>主管单位：{{item.majorCharge}}</p>
                        <p>现地址：{{item.enterpriseAddress}}</p>
                    </div>
                </div>
            </router-link>
        </div>    
