<template>
    <div class="home-container">
        <header id="header" class="mui-bar mui-bar-nav">
            ...
            <a class="mui-pull-right mui-icon mui-icon-right-nav" @click="addInspectionCompanyInfo">添加</a>
        </header>
        <div class="card-container">
            <form class="mui-input-group">
                <div class="mui-input-row" data-options='{"type":"date"}' @click="openPicker('NJNF_NJ',$event)">
                    <label data-options='{"type":"date"}'>年份：</label>
                    <input type="text" id="NJNF_NJ" v-model="pd.NJNF_NJ" data-options='{"type":"date" }' readonly="readonly">
                </div>
...
data() {
    return {
        ...
        QIYEXINXIGUANLI_ID: this.$route.params.id,			//父ID
        QIYEXINXIGUANLIMX_ID: '',	//主键ID
    }
},
...
methods: {
    getpd(){
        var data = {
            QIYEXINXIGUANLI_ID: this.QIYEXINXIGUANLI_ID, tm: new Date().getTime() 
        }
        this.$http.post('qiyexinxiguanli/goEdit',data,{withCredentials: true}).then(
            result => {
                if( result.body.result == "success" ){
                    for (const key in result.body.pd) {
                        if (result.body.pd.hasOwnProperty(key)) {
                            var value = result.body.pd[key];
                            this.$set(this.pd,key + "_NJ",value);
                            // this.pd[key + "_NJ"] = value;
                        }
                    }
                }
                else{
    ...
    addInspectionCompanyInfo(){
        if (this.pd.NJNF_NJ == "" || this.pd.NJNF_NJ == undefined){
            ...
            this.$http.post('qiyexinxiguanlimx/add',InspectionCompanyInfo,{withCredentials: true}).then(
                result => {
                    if( result.body.result == "success" ){
    ...
    goBack(){
        //点击后退
        this.$router.go(-1);
        // this.$router.replace('/companymanager/inspectioncompanyinfolist/'+this.QIYENIANJIAN_ID );
    },
    openPicker(id,$event){
        var that = this;
